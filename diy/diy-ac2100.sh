...
sed -i '/VERSION_NUMBER:=$(call qstrip,$(CONFIG_VERSION_NUMBER))/,+1c VERSION_NUMBER := Dswang Build $(shell TZ=UTC-8 date "+%Y-%m-%d") @ Firmware r$(REVISION)' include/version.mk
